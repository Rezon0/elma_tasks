...
func Solution(A []uint) uint { // метод поиска элемента без пары
	var (
		flag bool // флаг для отметки элемента с парой
		val  uint // переменная для хранения значения
	)
	for i := 0; i < len(A); i++ {
...
func main() {
	...
	array = make([]uint, N, 1000000) // инициализация массива

	for i := 0; uint(i) < N; i++ { // ввод элементов массива
