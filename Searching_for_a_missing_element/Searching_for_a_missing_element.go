...
func Solution(A []uint32) uint32 {
	var result uint32 // переменная для возвращения значения метода Solution
	...
	for i := 1; i <= len(A)+1; i++ { // цикл перебора чисел от [1 до N+1]
		...
		for j := 0; j < len(A); j++ { // цикл для перебора элементов массива
			result = uint32(i)     // записываем значение в result, если не выйдем досрочно из цикла, значит этот элемент наш
			if A[j] == uint32(i) { // проверяем есть ли такой элемент в массиве
				flag = true
				break
			}
		}
